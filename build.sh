#!/bin/bash
# Сборка и пуш образа planr-site в registry (по образцу versionControl/build.sh),
...
#
# Использование:
#   ./build.sh -r="registry.planr.cloud/site" -v="26.7.16" -c="$(git log -1 --pretty=%h)"
#
# Флаги:
#   -r, --registry  базовый путь registry (обязателен), напр. registry.planr.cloud/site
#   -v, --version   тег версии образа (обязателен)
#   -c, --commit    короткий хеш коммита (прокидывается в образ как build-arg version-хвост, опционально)
...
docker buildx build --platform "${PLATFORM}" -f Dockerfile \
    ...
    -t "${REGISTRY}"/planr-site-ui:"${VERSION}" --push .

echo "Готово: planr-site-ui:${VERSION} → ${REGISTRY} (${PLATFORM}, commit ${HASH})"
